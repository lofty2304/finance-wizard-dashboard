...
if st.button("Run Prediction"):
    ...
    try:
        ...
        X_train, y_train = [], []
        for i in range(10, len(scaled_data) - days_ahead):
            X_train.append(scaled_data[i-10:i, 0])
            y_train.append(scaled_data[i + days_ahead, 0])
        ...
        lower, upper = pred_price * 0.97, pred_price * 1.03
    ...
    st.write("⚙️ Using XGBoost Regressor")
    try:
        ...
        df["target"] = df["price"].shift(-days_ahead)

        X = df[features][:-days_ahead]
        y = df["target"][:-days_ahead]

        model = XGBRegressor(n_estimators=100)
        model.fit(X, y)
        pred_price = model.predict(df[features].iloc[[-1]])[0]
        lower, upper = pred_price * 0.95, pred_price * 1.05
